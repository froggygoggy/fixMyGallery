...
test('SessionController applyAction returns UI-friendly response', async () => {
  ...
  const controller = new SessionController(new ProcessSessionStepUseCase(cleanupRepo, appRepo));
  const vm = await controller.applyAction({
    state: start,
    selectedMediaIds: ['m1'],
  ...
  assert.equal(vm.nextState.status, 'goal_reached');
  assert.equal(vm.trashCount, 1);
  assert.equal(vm.undoHistoryCount, 1);
});
